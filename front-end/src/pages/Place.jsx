...
const Place = () => {
  ...
  const [checkiout, setCheckout] = useState("");
  ...
  const handleBooking = (e) => {
    ...
    if (checkin && checkiout && guests) {
      console.log("Fez uma reserva!");
    } else {
      alert("Preencha todos as informações antes de fazer uma reserva!");
    }
  };
  ...
  return (
    <section>
      <div className="mx-auto flex max-w-7xl flex-col gap-4 p-4 sm:gap-6 sm:p-8">
        ...
        {/* Grade de imagem*/}
        <div className="relative grid aspect-square gap-4 overflow-hidden rounded-2xl sm:aspect-[16/6.1] sm:grid-cols-[2fr_1fr] sm:grid-rows-2">
          {place.photos
            .filter((photo, index) => index < 3)
            .map((photo, index) => (
              <img
                className={`${index === 0 ? "row-span-2 h-full object-center" : ""} aspect-square w-full cursor-pointer transition hover:opacity-75 sm:aspect-video sm:object-cover`}
                src={photo}
          ...
          <div
            className="absolute right-2 bottom-2 flex cursor-pointer gap-1 rounded-xl border border-black bg-white px-2 py-1 transition hover:scale-105"
            onClick={() => setOverlay(true)}
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
